...
| 条形码生成 | `/barcode/` | Code128（含 CodeC 压缩）/ EAN-13 / UPC-A / EAN-8，自动校验位（python-barcode 位串比对验证），SVG 预览 + PNG 下载 |
| 图片加水印 | `/watermark/` | 批量文字水印（≤20 张）：字号/透明度/位置/旋转/颜色/斜向平铺可调，保持原格式，ZIP 下载 |
| CSS 渐变生成 | `/gradient/` | 线性/径向渐变，最多 4 色标自由定位，实时预览，一键复制 CSS |
...
另有十二篇 SEO 使用指南（`/guides/`，中英双语）：ICO 文件完全指南、Favicon 完全指南、暗色图标设计指南、图片压缩指南、WebP 格式指南、配色原理指南、App 图标尺寸速查、PNG 转 ICO 指南、Favicon 不显示排查、透明背景图标指南、无障碍对比度指南、二维码扫不出来排查。
