...
- 指南 9：ico-file-guide、favicon-guide、dark-mode-icons、image-compression-guide、webp-guide、color-guide、app-icon-sizes、**png-to-ico-guide**、**favicon-not-showing-guide**（均中英双语 `.lb-zh`/`.lb-en` 双块）
- 信任页：about / privacy / contact + 404（noindex）

**新增页面 checklist**：`sitemap.xml`（hreflang 三连 + priority）→ `llms.txt` → guides 索引卡（如指南）→ `site.js` 词典（`gd_*` 键）→ 相关工具页反向内链 → `<title>` 双语 meta + JSON-LD。
